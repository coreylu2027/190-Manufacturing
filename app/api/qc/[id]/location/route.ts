...
import { updateQualityLocation } from "@/lib/manufacturing";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(await updateQualityLocation(requirementId, parsed.data.location, currentUser));
  } catch (error) {
