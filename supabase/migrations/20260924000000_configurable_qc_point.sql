-- Configurable QC point. By default QC and finishing come after every
-- operation except threaded inserts. An admin can instead place them after OP N
-- (requirements.qc_after_operation): OP1..OPN are inspected, and later
-- operations wait for the QC pass and any finishing. The sync never writes it.
--
-- Replaces, from their latest definitions, every function that classified
-- operations by the Threaded Insert machine: routing initialization
-- (20260918162451), the On Robot guard (production/20260905_part_locations),
-- and the override state and apply RPC (20260923040000).

alter table manufacturing.requirements add column if not exists qc_after_operation smallint
  check (qc_after_operation between 1 and 4);

-- Mirrors isPostQcOperation in lib/manufacturing-workflow.ts.
create or replace function manufacturing.is_post_qc_operation(p_machine text, p_operation_number text, p_qc_after smallint)
returns boolean language sql immutable set search_path = '' as $$
  select case when p_qc_after is null then lower(btrim(coalesce(p_machine, ''))) = 'threaded insert'
    else coalesce(substring(upper(coalesce(p_operation_number, '')) from '^OP([0-9]+)$')::integer, 2147483647) > p_qc_after end;
$$;
revoke all on function manufacturing.is_post_qc_operation(text, text, smallint) from public, anon, authenticated, service_role;
...
begin
  ...
  loop
    ...
    where o.id=any(new_operation_ids) and o.work_type='Manufacturing'
      and (not manufacturing.is_post_qc_operation(o.machine, o.operation_number, requirement.qc_after_operation) or (
      ...
      and not exists (
        ...
        where earlier.requirement_id=requirement.id and earlier.active_in_routing
          ...
          and manufacturing.is_post_qc_operation(earlier.machine, earlier.operation_number, requirement.qc_after_operation)
            = manufacturing.is_post_qc_operation(o.machine, o.operation_number, requirement.qc_after_operation)
      ...
      update manufacturing.requirements r set status=case
        when not exists (select 1 from manufacturing.operations o
          where o.requirement_id=r.id and o.active_in_routing and o.work_type='Manufacturing'
            and o.operation_number='OP1' and not manufacturing.is_post_qc_operation(o.machine, o.operation_number, r.qc_after_operation)) then 'Needs Triage'
    ...
    if review_result is distinct from 'passed'
      ...
      or not exists(select 1 from (
        ...
        where o.requirement_id = target_requirement_id and o.active_in_routing
          ...
          and not manufacturing.is_post_qc_operation(o.machine, o.operation_number, (before_row->>'qc_after_operation')::smallint)
...
language sql stable set search_path = '' as $$
  ...
  select jsonb_build_object(
    'requirement', (select jsonb_build_object('id', id, 'production_key', production_key, 'part_id', part_id,
      ...
      'qc_outcome', qc_outcome, 'part_location', part_location, 'qc_after_operation', qc_after_operation) from r),
...
begin
  ...
  for item in select value from jsonb_array_elements(p_changes) loop
    ...
    allowed := case v_entity
      when 'requirements' then array['required_quantity','finishing','machine_op1','machine_op2','machine_op3','machine_op4','status','qc_outcome','off_the_shelf','qc_after_operation']
    ...
    if (v_entity = 'requirements' and patch ? 'qc_outcome' and patch->>'qc_outcome' is distinct from 'Not Inspected')
      ...
      or (patch ? 'off_the_shelf' and (jsonb_typeof(patch->'off_the_shelf') is distinct from 'boolean'
        or patch->'off_the_shelf' = before_row->'off_the_shelf'))
      or (patch ? 'qc_after_operation' and (patch->'qc_after_operation' = before_row->'qc_after_operation'
        or not (patch->'qc_after_operation' = 'null'::jsonb or (jsonb_typeof(patch->'qc_after_operation') = 'number'
          and (patch->>'qc_after_operation')::numeric in (1, 2, 3, 4))))) then
      raise exception 'Invalid engineering override change';
    end if;
    if patch ? 'qc_after_operation' then
      -- Moving QC reclassifies inspected work, so a passed review must be undone first.
      if target.obsolete or target.active_in_bom is distinct from true or target.off_the_shelf then
        raise sqlstate 'PT409' using message = 'Only active, manufactured requirements can move QC';
      end if;
      if target.qc_outcome = 'Passed' then
        raise sqlstate 'PT409' using message = 'Undo the passed QC review before changing when QC happens';
      end if;
      insert into manufacturing.engineering_override_events(request_id,entity,row_id,field,action,value,reason,actor,actor_name)
        values(p_request_id, 'requirements', v_row_id, 'qc_after_operation',
          case when patch->'qc_after_operation' = 'null'::jsonb then 'cleared' else 'set' end, patch->'qc_after_operation',
          coalesce(p_reason, ''), p_actor, coalesce(actor_name, p_actor::text));
    end if;
    ...
    if v_entity = 'operations' then
      ...
      if patch ?| array['completed_quantity','quantity_ledger','machinist'] then
        if not (patch ? 'completed_quantity' and patch ? 'quantity_ledger') or not quantity_overridden
          ...
          or manufacturing.is_post_qc_operation(before_row->>'machine', before_row->>'operation_number', target.qc_after_operation)
    ...
    if v_entity in ('requirements','parts') then
      for v_field in select k from jsonb_object_keys(patch) k
          where k not in ('status','qc_outcome','off_the_shelf','off_the_shelf_changed_by','off_the_shelf_changed_at','qc_after_operation') loop
